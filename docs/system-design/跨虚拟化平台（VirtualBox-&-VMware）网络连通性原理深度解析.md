...
# 跨虚拟化平台（VirtualBox & VMware）

**Box 虚拟机 (VirtualBox)** 与 **VM 虚拟机 (VMware)** 处于不同的虚拟子网（`192.168.33.0/24` 和 `192.168.66.0/24`），且使用了不同的网络模式（Host-Only 与 NAT）。它们之所以能 ping 通，核心在于**宿主机（Host OS）充当了“软路由器”的角色**。
